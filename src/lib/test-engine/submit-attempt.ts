...
export async function submitAttempt(input: unknown) {
  ...
  const { data: attempt, error: attemptError } = await db
    .from("test_attempts")
    .select(
      "id, user_id, guest_session_id, test_id, status, started_at, expires_at, submitted_at, score, max_score, correct_count, wrong_count, unanswered_count, time_taken_seconds, tests(name)",
    )
